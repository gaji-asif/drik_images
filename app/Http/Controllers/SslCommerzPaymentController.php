...
//   dd($request); 
        if(is_null(auth()->user())) {
            $request->session()->put('url','checkout');
        ...
        $post_data['cus_country'] = $request->city;
        ...
        foreach ($request->images as $key=>$item) { 
            ...
            $purchaseDetails->image_id = $item;
            $purchaseDetails->price = $request->images_price[$key];
        ...
        if ($order_detials->status == 'Pending') {
            ...
            if ($validation == TRUE) {
                ...
                */
                ...
                $update_purchase = DB::table('purchases')
                    ->where('transaction_id', $tran_id)
                    ->update(['payment_status' => 'Failed','payment_method' => $request->input('card_type')]);    
